...
class DisksCollector(QueryCollector):
    ...
    async def parse(self, data: Dict[str, Any]) -> List[EntityUpdate]:
        ...
        for d in devices:
            ...
            # Temperature sensor (only create if valid temperature exists)
            ...
            # Disk size sensors (size, used, free in TB)
            BYTES_PER_SECTOR = 1024
            BYTES_IN_TB = 1_000_000_000_000

            disk_name = d.get('name', '')
            size_value = d.get('size', 0)
            fs_size_value = d.get('fsSize', 0)
            fs_used_value = d.get('fsUsed', 0)
            fs_free_value = d.get('fsFree', 0)

            try:
                # Calculate disk sizes in TB
                disk_size_tb = round((int(size_value) * BYTES_PER_SECTOR) / BYTES_IN_TB, 2) if size_value else 0
                disk_used_tb = round((int(fs_used_value) * BYTES_PER_SECTOR) / BYTES_IN_TB, 2) if fs_used_value else 0
                disk_free_tb = round((int(fs_free_value) * BYTES_PER_SECTOR) / BYTES_IN_TB, 2) if fs_free_value else 0
                ...
                if disk_size_tb:
                    updates.append(
                        EntityUpdate(
                            ...
                            state=disk_size_tb,
                            retain=True,
                            unique_id_suffix=f"{disk_name}_size",
                        )
                    )
                ...
                if disk_used_tb:
                    updates.append(
                        EntityUpdate(
                            ...
                            state=disk_used_tb,
                            retain=True,
                            unique_id_suffix=f"{disk_name}_used",
                        )
                    )
                ...
                if disk_free_tb:
                    updates.append(
                        EntityUpdate(
                            ...
                            state=disk_free_tb,
                            retain=True,
                            unique_id_suffix=f"{disk_name}_free",
                        )
                    )
